 ...
 */
const checkQuotaAvailability = async (planId, requestedQuota = 1) => {
 ...
 */
const allocateQuota = async (planId, quotaAmount = 1) => {
 ...
 * @returns {Promise<Object>} Updated quota information
 */
const releaseQuota = async (planId, quotaAmount = 1) => {
  // Use transaction to ensure consistency
  return await prisma.$transaction(async (tx) => {
 ...
 */
const getQuotaOverview = async (options = {}) => {
  ...
  return plans.map((plan) => ({
    ...
    usedQuota: plan.usedQuota,
    availableQuota: plan.totalQuota - plan.usedQuota,
    utilizationPercentage: Math.round((plan.usedQuota / plan.totalQuota) * 100),
 ...
 */
const getQuotaStatistics = async () => {
 ...
 */
const validateQuotaOperation = async (planId, operation, amount = 1) => {
  const plan = await prisma.servicePlan.findUnique({
    where: { id: planId, isActive: true },
    select: {
      id: true,
